...
This deposit uses scoped, file-level licensing:
...
`LICENSE_MATRIX.tsv` is generated during the deterministic packet build and is
the controlling path-by-path map. A directory name or archive-level metadata
statement must not be interpreted as broadening the licence assigned to an
individual file.
